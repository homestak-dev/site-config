...
setup() {
    ...
    rm -f "$SITE_CONFIG_DIR/specs/_test_invalid.yaml"
    rm -f "$SITE_CONFIG_DIR/postures/_test_invalid.yaml"
}
...
@test "validates a valid posture file" {
    run "$SCRIPT" "$SITE_CONFIG_DIR/postures/dev.yaml"
    [ "$status" -eq 0 ]
}
...
@test "--json outputs valid JSON for an invalid file" {
    ...
    cat > "$SITE_CONFIG_DIR/specs/_test_invalid.yaml" <<'EOF'
schema_version: 999
bogus_field: true
EOF
    run "$SCRIPT" --json "$SITE_CONFIG_DIR/specs/_test_invalid.yaml"
    [ "$status" -eq 1 ]
...
schema_version: 999
EOF
    run "$SCRIPT" "$SITE_CONFIG_DIR/specs/_test_invalid.yaml"
    [ "$status" -eq 1 ]
}

@test "detects invalid posture (unexpected field)" {
    cat > "$SITE_CONFIG_DIR/postures/_test_invalid.yaml" <<'EOF'
auth:
  method: network
completely_bogus_field: 42
EOF
    run "$SCRIPT" "$SITE_CONFIG_DIR/postures/_test_invalid.yaml"
    [ "$status" -eq 1 ]
}
...
@test "resolves spec schema for specs/ files" {
    run "$SCRIPT" "$SITE_CONFIG_DIR/specs/pve.yaml"
    [ "$status" -eq 0 ]
}

@test "resolves posture schema for postures/ files" {
    run "$SCRIPT" "$SITE_CONFIG_DIR/postures/prod.yaml"
    [ "$status" -eq 0 ]
}
